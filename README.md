...
| **Light gizmos** | Every light draws its shape in the viewport — a range sphere for point lights, an angle-and-range cone for spots, aim arrows for the sun — tinted by the light's colour. Select one and drag the handles on it to scale range, open or close the cone, or re-aim, without leaving the viewport |
...
- **Directional, point, and spot lights** in one GPU light buffer (`std430` SSBO) — the sun is a placeable entity you aim with its rotation; point/spot have range and cone falloff, and each light has a per-entity **Cast Shadows** toggle. In the editor every light shows an editable gizmo — range sphere, spot cone, or aim arrow — with drag handles for range, cone angle, and aim
