...
#include "TartarusVersion.h" // generated by CMake - identifies the build in a crash report
...
DWORD WINAPI Watchdog(void*) {
    ...
    // The version is on this line so a pasted console tail identifies the build on its own —
    // a stack in a dump is worthless without knowing which binary produced it.
    wsprintfA(line, "\n[CrashHandler] Tartarus Engine " TARTARUS_VERSION " (" TARTARUS_GIT_DESCRIBE
                    ") %s (0x%08lx) - minidump %s: %ls\n", g_Reason,
