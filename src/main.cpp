...
#include "TartarusVersion.h" // generated by CMake - engine version + git describe, shown in About
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        std::vector<BulletHoleList::Placed> placedHoles;
        ...
        // The crosshair dot: the plain white centre dot, gone with a weapon in hand - its laser
        // (below, in the world) is the aim point then.
        auto crosshairDot = [&](const glm::mat4&, const glm::mat4&, int, int) {
            CrosshairOverlay::Dot dot;
            if (firstPersonPresentation.IsActive() && firstPersonPresentation.IsEquipped()) dot.Visible = false;
            return dot;
        };
        // The weapon's world effects for one view, drawn inside the scene pass (after the world,
        // before the arms and gun, so they cover the beam where it leaves the muzzle): this Play's
        // bullet holes and the laser. Empty (nothing drawn) outside first-person Play.
        auto weaponOverlay = [&](bool on, const glm::mat4& v, const glm::mat4& p, const glm::vec3& eye,
...
#endif
                char b[256];
                snprintf(b, sizeof(b), "Engine: Tartarus Engine %s (%s)",
                         TARTARUS_VERSION, TARTARUS_GIT_DESCRIBE);
                report(b);
