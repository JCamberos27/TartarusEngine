...
| **Sight Alignment** | With sights up, IK holds the weapon's sight line on the centre of the screen, so it aims true without hand-tuning. It works on the animated pose before the other layers, so recoil, sway and breathing still move the sights.<br><br>**Source** sets where the line comes from:<br>- **Aim Clip** (the default) reads the line from the ADS state's clip when Play starts. It keeps that clip's sight picture exactly and holds it through transitions, so use it only when the Aim clip already lines up.<br>- **Manual** takes rear and front sight points on the weapon rig, for weapons whose Aim clip doesn't line up. The AK uses Manual: its Aim clip holds the gun left of centre with the post off the notch, and its points (rear notch and front post tip, in the `root` bone's space) come from `work/sight_align_probe.cpp`. The Console prints the Aim clip's line as another starting point.<br><br>**Eye Distance** moves the sights nearer or further (0 keeps the pose's distance). When Play starts, a self-check logs how precisely the sights hold on the real rig. |
